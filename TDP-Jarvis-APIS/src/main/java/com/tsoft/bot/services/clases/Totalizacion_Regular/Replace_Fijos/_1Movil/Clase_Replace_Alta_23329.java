...
public class Clase_Replace_Alta_23329 {
    ...
    public static String showData(String status) throws Throwable {
        ...
        String []escenario1 = {"23329", "Movistar Total 13GB TV Estándar Digital HD 200Mbps", "275.0", "200000", "43.0",
                                "13000", "232.0", "43.0", "De alta velocidad para compartir PasaGigas y usar en America y Europa."};

        ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 8, 2, resultado);
        ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 8, 3, "10. Replace + (Alta)");
        ...
        {
            ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 8, 13, "PASS");
        }
        else
        {
            ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 8, 13, "FAILED");
        }

        for (int i = 0; i < escenario1.length; i++)
        {
            ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 8, i + 4, "" + escenario1[i] + ": " + resultado.contains(escenario1[i]) + i);
        }
