...
public class Clase_Replace_Alta_23323 {
    ...
    public static String showData(String status) throws Throwable {
        ...
        String []escenario1 = {"23323", "Movistar Total 13GB TV Estándar Digital HD 50Mbps", "195.0", "50000", "43.0",
                                "13000", "152.0", "43.0", "De alta velocidad para compartir PasaGigas y usar en America y Europa."};

        ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 7, 2, resultado);
        ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 7, 3, "10. Replace + (Alta)");
        ...
        {
            ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 7, 13, "PASS");
        }
        else
        {
            ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 7, 13, "FAILED");
        }

        for (int i = 0; i < escenario1.length; i++)
        {
            ExcelReader.writeCellValue(EXCEL_WEB, HOJA, 7, i + 4, "" + escenario1[i] + ": " + resultado.contains(escenario1[i]) + i);
        }
